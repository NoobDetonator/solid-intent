...
export function serializeParameters(parameters: Record<string, number>): string {
...
function createLoaders({ repositoryRoot, projectsRoot }: ServerPaths) {
  ...
  return { loadProject, updateParameters, resolveWorkspaceReference };
}
...
export function registerApiRoutes(
  ...
  app.get("/api/projects", async (_request, response, next) => {
    try {
      ...
      for (const entry of entries) {
        ...
        const manifestPath = path.join(paths.projectsRoot, entry.name, "project.json");
  ...
  app.get("/api/projects/:projectId/artifacts/:artifactKey", async (request, response, next) => {
    try {
      assertProjectId(request.params.projectId);
      const projectDirectory = resolveInside(paths.projectsRoot, request.params.projectId);
      const manifest = await readJson<JsonObject>(path.join(projectDirectory, "project.json"));
      const artifacts = manifest.artifacts as Record<string, string>;
