"""Generate the dimensioned Raspberry Pi 5 enclosure drawing.
...
from raspberry_pi5_case import (
    BASE_VENT_X,
    BASE_VENT_Y_POSITIONS,
    LID_VENT_Y_POSITIONS,
    build_model,
)


PROJECT_ROOT = Path(__file__).resolve().parents[1]
PARAMETER_PATH = PROJECT_ROOT / "projects" / "raspberry_pi5_case" / "parameters.json"
PAGE_WIDTH = 420.0
PAGE_HEIGHT = 297.0
DRAWING_SCALE = 1.0
SVG_PATH = str(PROJECT_ROOT / "drawings" / "raspberry_pi5_case_dimensioned.svg")
DXF_PATH = str(PROJECT_ROOT / "drawings" / "raspberry_pi5_case_dimensioned.dxf")
DIMS_PATH = str(PROJECT_ROOT / "drawings" / "raspberry_pi5_case_dimensioned.dims.json")
...
def generate_drawing(parameters=None):
    """Generate the combined A3 base-and-lid engineering drawing for Pi 5."""
    if parameters is None:
        parameters = load_project_parameters()

    model = build_model(parameters)
    base = model["bodies"]["base"]
    lid = model["bodies"]["lid"]
    ...
    # Pi 5 lid top matches the base outer envelope (internal skirt).
    lid_outer_length = base_outer_length
    lid_outer_width = base_outer_width
    ...
    base_plan = project_view(
        base, (0.0, 0.0, 180.0), (0, 1, 0), base_look, base_plan_center
    )
    base_iso = project_view(
        base, (100.0, 100.0, 100.0), (0, 0, 1), base_look, (305.0, 215.0)
    )

    lid_look = (0.0, 0.0, lid_height / 2)
    lid_plan = project_view(
        lid, (0.0, 0.0, 150.0), (0, 1, 0), lid_look, lid_plan_center
    )
    ...
    add_annotation(
        Leader(
            base_plan_point(mount_right, mount_top),
            (121.0, 246.0, 0),
            f"4x DIA {parameters['mount_hole_diameter']:.1f} THRU",
            draft,
        ),
        "base_hole_callout",
    )
    add_annotation(
        Leader(
            base_plan_point(BASE_VENT_X, 0.0),
            (121.0, 215.0, 0),
            (
                f"{len(BASE_VENT_Y_POSITIONS)}x "
                f"{parameters['base_vent_length']:.1f} x "
                f"{parameters['base_vent_width']:.1f} VENT SLOTS"
            ),
            draft,
        ),
        "base_vent_callout",
    ...
    add_annotation(
        Leader(
            lid_plan_point(0.0, LID_VENT_Y_POSITIONS[0]),
            (350.0, 145.0, 0),
            (
                f"{len(LID_VENT_Y_POSITIONS)}x "
                f"{parameters['lid_vent_length']:.1f} x "
                f"{parameters['lid_vent_width']:.1f} VENTS"
            ),
            draft,
            text_side="right",
        ),
    ...
    notes = (
        ...
        (
            "PROTOTYPE - VERIFY CONNECTOR WINDOWS ON PHYSICAL PI 5 BOARD",
            (105.0, 24.0, 0),
            "general_note_2",
        ),
        (
            ...
            f"{parameters['lid_fit_clearance']:.2f}",
            (105.0, 18.0, 0),
            "general_note_3",
        ),
    )
